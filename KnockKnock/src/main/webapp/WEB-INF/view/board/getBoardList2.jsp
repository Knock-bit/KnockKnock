...
<div id="container">
   ...
   <c:if test="${not empty getBoardList}">
      ...
      <tr style="cursor:pointer;" onclick="location.href='${cp}/board/getBoard.do?bIdx=${board.bIdx }'">
         ...
         <td class="text-center">${board.uIdx}</td>
   ...
   <c:if test="${empty getBoardList}">
      <tr>
         <td colspan="7" class="text-center">현재 게시글이 존재하지 않습니다.</td>
      </tr>
   </c:if>
   </table>
   ...
   <div>
      <input type="button" value="게시글등록" onclick="moveInsert_board(this.form)"
         style="width: 100px; color:white; background-color: #0a3a18; border-radius: 35px;"
      >
      <input type="button" value="내글보기" onclick="myView_board(this.form)"
         style="width: 100px; color:white; background-color: #0a3a18; border-radius: 35px;"
      >
   </div>   
   </form>
